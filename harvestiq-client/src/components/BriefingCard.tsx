...
function cleanBriefingText(text: string, language: string): string {
  ...
  if (text.includes("Morning briefing (deterministic):") || text.includes("Farm ID:")) {
    ...
    const gddMatch = text.match(/-\s*Current GDD:\s*(.*?)(?=\s+-\s+|$)/i);
    const fsiMatch = text.match(/-\s*FSI:\s*(.*?)(?=\s+-\s+|$)/i);

    const crop = cropMatch ? cropMatch[1].trim() : "";
    const stage = stageMatch ? stageMatch[1].trim() : "";
    const gdd = gddMatch ? gddMatch[1].trim() : "";
    const fsi = fsiMatch ? fsiMatch[1].trim() : "";

    if (language === "hi") {
      let insight = "आपके खेत की दैनिक स्थिति का विवरण यहाँ है। ";
      if (crop) insight += `आपकी ${crop} की फसल वर्तमान में ${stage || "विकास"} चरण में है। `;
      if (gdd) insight += `फसल का संचयी तापमान विकास (GDD) ${gdd} है। `;
      if (fsi) insight += `खेत का तनाव सूचकांक (FSI) ${fsi} दर्ज किया गया है, और किसी भी तनावपूर्ण स्थिति पर नज़र रखी जा रही है।`;
      return insight;
    } else {
      ...
      if (gdd) insight += `The cumulative Growing Degree Days (GDD) is ${gdd}. `;
      if (fsi) insight += `The Field Stress Index (FSI) is ${fsi}, indicating stable growing conditions.`;
      return insight;
    }
  }
