...
from app.models.farm_db_schemas import (
    FarmCreateSchema,
    PlotCreateSchema,
...
class SyncService:
    ...
    async def replay_batch(self, user_id: str, payload: SyncBatchRequest) -> SyncBatchResponse:
        ...
        for operation in payload.operations:
            ...
            try:
                ...
                detail, server_id = await self._execute_operation(user_id, operation.operation_type, op_payload)
    ...
    async def _execute_operation(self, user_id: str, operation_type: str, payload: dict) -> tuple[str, Optional[str]]:
        ...
        if operation_type == "MARK_ALERT_READ":
            ...
            if result.matched_count == 0:
                raise unprocessable_entity("Alert not found")
            return f"alert {alert_id} marked read", None
        ...
        if operation_type == "TRIGGER_SOS":
            ...
            result = await sos_service.trigger(user_id, sos_payload)
