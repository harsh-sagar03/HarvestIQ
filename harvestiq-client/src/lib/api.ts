...
import { syncLocalKnowledge, type KnowledgeSyncPayload } from "@/lib/sqliteDb";
import {
  createFarm,
  deleteFarm,
  createPlot,
...
export async function apiRequest<T>(
  ...
  console.log(`[TRACE:apiRequest] ENTERED | path=${path} | navigator.onLine=${typeof navigator !== "undefined" ? navigator.onLine : "SSR"}`);
  /** Bump when verifying browser loads latest offline fallback logic */
  const API_TRACE_BUILD = "offline-trace-v3";
  if (typeof window !== "undefined") {
    (window as Window & { __HIQ_API_TRACE_BUILD?: string }).__HIQ_API_TRACE_BUILD = API_TRACE_BUILD;
  }
  ...
  if (networkError || isServerError || isOffline401 || isNotFound) {
    ...
    if (method === "POST" || method === "PUT" || method === "DELETE") {
      if (path.includes("/sos/trigger") && method === "POST" && typeof rest.body === "string") {
        ...
        return {
          ...
          delivery_status: "QUEUED_OFFLINE",
      ...
      if (path.includes("/alerts/") && path.includes("/read") && method === "PUT") {
        const alertId = path.split("/alerts/")[1].split("/read")[0];
...
export const api = {
  ...
  triggerSos: (payload: {
    ...
    apiRequest<SosTriggerResult>("/api/v1/sos/trigger", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
