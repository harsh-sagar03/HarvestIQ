...
def confirm_disease_detection(
    ...
    if disease_tag in allowed:
        return disease_tag, DISEASE_STATUS_CONFIRMED

    for candidate in allowed:
        if candidate in disease_tag or disease_tag in candidate:
            return disease_tag, DISEASE_STATUS_CONFIRMED

    # High-confidence vision detections (≥ 80%) are approved even without
    # an exact regional allowlist match — the model is confident enough.
    if confidence >= 0.80:
        return disease_tag, DISEASE_STATUS_CONFIRMED

    return disease_tag, DISEASE_STATUS_REJECTED
